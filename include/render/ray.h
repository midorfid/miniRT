...
static inline vec3_t    sample_square_stratified(int s_i, int s_j, int rec1p_sqrt_spp) {
    // Returns the vector to a random point in the square sub-pixel specified by grid
    // indices s_i and s_j, for an idealized unit square pixel [-.5,-.5] to [+.5,+.5].
    double y = ((random_double_nolimits() + s_i) * rec1p_sqrt_spp) - 0.5;
    double x = ((random_double_nolimits() + s_j) * rec1p_sqrt_spp) - 0.5;

    return vec3(x, y, 0.0);
}

static inline ray_t get_ray(int height, int width, int s_i, int s_j, const render_plane_t *render_p, point3_t center, const defocus_blur_t *lens, int rec1p_sqrt_spp) {
